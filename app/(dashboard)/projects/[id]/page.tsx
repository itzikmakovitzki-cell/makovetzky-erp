  ...
  return (
    <section className="flex flex-col gap-5">
      <div>
        <Link
          href="/projects"
          className="inline-flex min-h-10 cursor-pointer items-center gap-1 rounded-lg px-2 text-xs font-semibold text-muted-foreground transition-colors hover:bg-white/70 hover:text-foreground"
        >
      ...
      <header className="relative overflow-hidden rounded-[1.75rem] border border-white/80 bg-white/95 p-5 shadow-[0_14px_44px_rgba(31,41,55,0.09)] md:p-7">
        ...
        <div className="relative flex flex-wrap items-start justify-between gap-5">
          <div className="min-w-0">
            <div className="mb-2 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-bold text-primary">
              <FolderKanban className="size-3.5" /> פרויקט
            </div>
            <h1 className="text-2xl font-black tracking-tight text-brand-navy md:text-3xl">
              {deal.name}
            </h1>
          ...
          <div className="flex max-w-md shrink-0 flex-wrap items-center justify-end gap-2">
            {isAdmin ? (
            ...
            <Link
              ...
              className="inline-flex min-h-10 cursor-pointer items-center gap-1.5 rounded-xl border border-input bg-card px-3 py-2 text-xs font-semibold transition-colors hover:bg-accent"
        ...
        <div className="relative mt-6 grid grid-cols-2 gap-3 md:max-w-xl">
        ...
        {deal.notes && (
          <div className="relative mt-5 flex gap-2 rounded-2xl border bg-[#fbfaf7] px-4 py-3 text-sm">
      ...
      <section className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">העבודה בפועל</p>
          <h2 className="mt-1 text-xl font-extrabold tracking-tight text-brand-navy">היתרים בפרויקט</h2>
        </div>
        ...
        <table>
          ...
          <tbody>
            ...
            {deal.permits.map((p) => {
              ...
              return (
                <tr key={p.id} className="hover:bg-primary/[0.035]">
                  <td>
                    <Link
                      href={`/permits/${p.id}`}
...
function Stat({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-border/60 bg-[#fbfaf7] p-3.5">
      <div className="text-[11px] font-semibold text-muted-foreground">{label}</div>
      <div className="mt-1 text-lg font-extrabold text-brand-navy tabular-nums">{value}</div>
    </div>
  );
}
